...
`schemas/evaluation.py` exposes schema version `1.0.0`, a JSON Schema for structured-output provider APIs, exact evidence validation, and deterministic scoring. The evaluator is provider-neutral: the current local default is Gemini (`gemini-3.5-flash-lite`) using native JSON Schema output, while the OpenAI Responses adapter remains available for configuration-selected comparisons. Every response is validated before it reaches business logic. A unique verbatim quote's offsets and the published weighted overall score are canonicalized deterministically; all other unsupported evidence is rejected. Output includes provider/model, prompt SHA-256, schema/evaluation/rubric/prompt versions, request ID when available, input/output tokens, and latency for immutable analysis accounting.
