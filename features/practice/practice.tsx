...
export type SpeakerNotesMode = "quick_glances" | "always" | "hidden";
...
export function Briefing({ go, assignment }: AssignmentProps) {
  if (!assignment) return <AssignmentUnavailable />; const challenge = assignment.challenge;
  return <div className="page briefing narrow"><button className="back" onClick={() => go("home")}>← Home</button><Eyebrow>{assignmentTitle(assignment)} · version {challenge.version}</Eyebrow><h1>Make your case</h1><p className="prompt">{challenge.prompt}</p><div className="meta-row"><div><span>Target duration</span><strong>{Math.round(challenge.targetDurationSeconds / 60)} minutes</strong></div><div><span>Difficulty</span><strong>{challenge.difficulty} / 5</strong></div><div><span>Skills</span><strong>{challenge.targetSkills.join(" · ")}</strong></div></div>{challenge.targetVocabulary.length > 0 && <Card className="challenge-vocabulary"><Eyebrow>Use naturally, not mechanically</Eyebrow><h2>Words to reach for</h2><p>{challenge.targetVocabulary.join(" · ")}</p><Button variant="secondary" onClick={() => go("vocabulary")}>Review in word bank</Button></Card>}<Button onClick={() => go("prepare")}>Prepare to speak <span>→</span></Button></div>;
}

export function Preparation({ go, assignment, notes, onNotesChange }: AssignmentProps & { notes: string; onNotesChange: (notes: string) => void }) {
  if (!assignment) return <AssignmentUnavailable />; const challenge = assignment.challenge;
  return <div className="page preparation"><div className="narrow"><button className="back" onClick={() => go("briefing")}>← Challenge</button><Eyebrow>Take a moment</Eyebrow><h1>Prepare your<br /><i>thinking.</i></h1><p>Use this space to shape your response. These notes are private and won’t be evaluated.</p><ol>{challenge.preparationGuidance.split(/\n|(?<=\.)\s+(?=[A-Z])/).filter(Boolean).map(step => <li key={step}>{step}</li>)}</ol><label className="notes-label">Your notes <span>Optional</span><textarea value={notes} onChange={event => onNotesChange(event.target.value)} placeholder="A few prompts for yourself…" /></label><Button onClick={() => go("speak")}>I’m ready to speak</Button></div></div>;
}

function SpeakerNotes({ notes, mode }: { notes: string; mode: SpeakerNotesMode }) {
  const [visible, setVisible] = useState(mode === "always"); const [cooldownUntil, setCooldownUntil] = useState(0); const [now, setNow] = useState(Date.now());
  useEffect(() => { setVisible(mode === "always"); setCooldownUntil(0); }, [mode]);
  useEffect(() => { if (mode !== "quick_glances" || !visible) return; const timer = window.setTimeout(() => { setVisible(false); setCooldownUntil(Date.now() + 20_000); }, 5_000); return () => window.clearTimeout(timer); }, [mode, visible]);
  useEffect(() => { if (mode !== "quick_glances" || cooldownUntil <= now) return; const timer = window.setInterval(() => setNow(Date.now()), 500); return () => window.clearInterval(timer); }, [cooldownUntil, mode, now]);
  if (!notes.trim() || mode === "hidden") return null;
  const remaining = Math.max(0, Math.ceil((cooldownUntil - now) / 1000));
  return <aside className={`speaker-notes ${visible ? "visible" : ""}`} aria-live="polite"><div><span>Your private notes</span>{visible && <p>{notes}</p>}</div>{mode === "quick_glances" && <Button variant="secondary" disabled={visible || remaining > 0} onClick={() => { setNow(Date.now()); setVisible(true); }}>{visible ? "Visible for 5 seconds" : remaining ? `Notes in ${remaining}s` : "Show notes for 5s"}</Button>}</aside>;
}

export function Speaking({ go, assignment, token, retryOfAttemptId, onQueued, notes, notesMode }: AssignmentProps & { token: string; retryOfAttemptId: string | null; onQueued: (attemptId: string) => void; notes: string; notesMode: SpeakerNotesMode }) {
