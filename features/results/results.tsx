...
import { EvidenceTracePlayer, type EvidenceMarker } from "./evidence-trace-player";
...
type AnnotatedWord = TranscriptWord & { index: number; tags: string[] };

function annotatedWords(result: AnalysisResultResponse): AnnotatedWord[] {
  ...
  return allWords.map((item, index) => ({ ...item, index, tags: [fillers.has(item.normalized_word.toLowerCase()) && "filler", repeated.has(item.normalized_word.toLowerCase()) && "repeat", corrections.has(index) && "correction"].filter((tag): tag is string => Boolean(tag)) }));
}

function annotatedTranscript({ result, evidence, selectedTime, onSelectTime }: { result: AnalysisResultResponse; evidence: AnnotatedWord[]; selectedTime: number | null; onSelectTime: (time: number) => void }) {
  if (!evidence.length) return <p>{result.transcript.text}</p>;
  return <div className="annotated-transcript" role="group" aria-label="Timestamped transcript">{evidence.map(item => {
    ...
    const selected = item.start_time != null && selectedTime != null && Math.abs(item.start_time - selectedTime) < 0.25;
    if (item.start_time == null) return <span key={`${item.index}-unknown`} className={`transcript-token ${tags}`} title={confidence}>{item.word}</span>;
    return <button key={`${item.index}-${item.start_time}`} type="button" className={`transcript-word transcript-token ${tags} ${selected ? "selected" : ""}`} onClick={() => onSelectTime(item.start_time as number)} aria-pressed={selected} title={`${timestamp(item.start_time)} · ${confidence}`}>{item.word}</button>;
  })}</div>;
}
...
export function Results({ go, token, attemptId, onRetry }: { go: (v: View) => void; token: string; attemptId: string | null; onRetry: (attemptId: string, attempt: AttemptResponse) => void }) {
  ...
  const responseCoverage = result.scorecard.dimensions?.response_coverage;
  const requiredWords = responseCoverage?.input?.minimum_word_count;
  const evidence = annotatedWords(result);
  const markers: EvidenceMarker[] = evidence.flatMap(item => item.start_time == null || item.end_time == null ? [] : item.tags.map(kind => ({ start: item.start_time as number, end: item.end_time as number, kind: kind as EvidenceMarker["kind"], label: kind === "filler" ? `Filler word: ${item.word}` : kind === "repeat" ? `Immediate repetition: ${item.word}` : `Possible self-correction: ${item.word}` })));
  return <div className="page results"><Eyebrow>Challenge review · {reviewAttempt ? `${Math.round(reviewAttempt.challenge.targetDurationSeconds / 60)} minute target` : "Completed attempt"}</Eyebrow><div className="result-head"><div><h1>{evaluation.primary_weakness.dimension === "structure" ? "Make the shape clearer." : "Build on this attempt."}</h1><p>{reviewAttempt?.challenge.prompt}</p>{responseCoverage?.score != null && responseCoverage.score < 100 && <p className="muted">Overall score is capped at {responseCoverage.score} until the transcript reaches {typeof requiredWords === "number" ? `${requiredWords} words` : "the response-coverage threshold"}. This prevents silence or a fragment from receiving delivery credit.</p>}</div><Score value={overall} /></div>
    <section className="split-section"><div><Eyebrow>Listen back</Eyebrow><h2>Your recording</h2><p className="muted">Private playback is available only until raw-audio retention ends.</p></div><Card>{playback ? <EvidenceTracePlayer source={playback} markers={markers} selectedTime={selectedTime} onSelectTime={setSelectedTime} /> : <p>Audio is no longer available, but your transcript and analysis remain.</p>}</Card></section>
    ...
    <section className="transcript"><Eyebrow>Timestamped transcript</Eyebrow>{annotatedTranscript({ result, evidence, selectedTime, onSelectTime: setSelectedTime })}<div className="annotation-key"><span className="filler">Filler word</span><span className="repeat">Immediate repetition</span><span className="correction">Possible self-correction</span></div><p className="muted">Select a timestamped word to seek the recording. Annotations are lexical heuristics from the transcript, not a diagnosis of stuttering, nervousness, or confidence.</p></section>
